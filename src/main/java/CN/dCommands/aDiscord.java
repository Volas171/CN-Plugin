...
public class aDiscord implements MessageCreateListener {
    ...
    public aDiscord(JSONObject data, DiscordApi api){
        this.data = data;
        this.api = api;
    }
            ...
            if (arg[0].startsWith("..") || arg[0].startsWith(data.getString("prefix"))) {
                ...
                switch (arg[0]) {
                    ...
                    case "rpk":
                        if (arg.length > 1 && arg[1].length() > 3) {
                            if (arg[1].startsWith("#") && Strings.canParseInt(arg[1].substring(1))) {
                                ...
                                Player p = playerGroup.getByID(id);
                                if (p == null) {
                                    event.getChannel().sendMessage(", Could not find player ID '" + id + "'.");                                    return;
                                }
                    ...
                    case "setTag":
                        if (arg.length > 2) {
                            ...
                            if (arg[1].startsWith("#") && arg[1].length() > 3 && Strings.canParseInt(arg[1].substring(1))){
                                ...
                                Player p = playerGroup.getByID(id);
                                if (p == null) {
                                    event.getChannel().sendMessage("[salmon]ST[white]: Could not find player ID `" + id + "`.");
                                    return;
                                }
                                uid = p.uuid;
                                tag = arg[2];
                                ...
                                event.getChannel().sendMessage("[salmon]ST[]: ID can only contain numbers!");
                            } else if (netServer.admins.getInfo(arg[1]).timesJoined > 0) {
                                //run
                                uid = arg[1];
                                tag = arg[2];
                                ...
                                event.getChannel().sendMessage("[salmon]ST[]: UUID not found!");
                            }

                            if (proceed) {
                                if (Main.database.containsKey(uid)) {
                                    if (!tag.contains("#")) {
                                        event.getChannel().sendMessage("[salmon]ST[white]: Discord tag must contain `#`! example: abc123#4567");
                                        return;
                                    } else if (tag.length() <= 5) {
                                        event.getChannel().sendMessage("[salmon]ST[white]: Discord tag must be at least 6 digits! example: abc123#4567");
                                        return;
                                    }
                                    ...
                                    event.getChannel().sendMessage("[salmon]ST[white]: Discord tag set to `[lightgray]" + tag + "[white]` for `[lightgray]" + uid +"[white]`.");
                                } else {
                                    event.getChannel().sendMessage("[salmon]ST[white]: Player not found in database.");
                                }
                            }
                        } else if (arg.length > 1) {
                            event.getChannel().sendMessage("[salmon]ST[]: You must provide discord tag!");
                        } else {
                            event.getChannel().sendMessage("[salmon]ST[]: Verifies player and adds discord tag using String. example /a #123 abc1234#1234");
                        }
                        break;
                    case "ban":
                        if (arg.length > 2) {
                    ...
                    case "pjl": //list of players joining and leaving
                        ...
                        if (Main.pjl.size > 50) {
                            for (int i = Main.pjl.size - 50; i < Main.pjl.size; i++) {
                        ...
                        if (arg.length > 1) {
                            switch (arg[1]) {
                                ...
                                case "off":
                                    ...
                                    break;
                                default:
                                    event.getChannel().sendMessage("Turn chat on or off.");
                            }
                        } else {
                            event.getChannel().sendMessage("Turns chat on/off, on/off");
                        }
                        break;
